...
def make_simulator():
    ...
    def sample_stop_prob(score, state, sim_parameters):
        ...
        #next_stop_prob = 1/(1+jax.numpy.exp((E - par_thresh_E)/0.5))
        #stop = np.random.binomial(1,next_stop_prob)
        #return stop
    ...
    def run(score, history, hits, alive_states, program_st, sim_parameters, step_count, fifos = None):
        ...
        if found_alt:
            ...
            interact_primal = program_st['y']['hits'][alt_index][3]
            program_st['y']['hits'][alt_index][3] = (not interact_primal)
    ...
    def simulator(par, split_prob, reuse_rvs = True):
        init = generate_random_init()
        program_st = stochasticTriple(0., None, 0.)
        
        sim_parameters = {'thresh_E': 0.5, 'split_prob':split_prob, 'thresh_x': par}
